...
function initWalletUI() {
  ...
  if (sidebarNav && !document.getElementById('sidebar-wallet-item')) {
    ...
    walletItem.innerHTML = `
      ...
      <span class="nav-item-text">المحفظة</span>
      <span class="sidebar-wallet-badge" id="sidebar-wallet-badge" style="margin-right: auto; background: rgba(59, 130, 246, 0.2); color: #60A5FA; padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 800;">0.00 ج.م</span>
    `;
...
function checkEmailVerificationBanner(user) {
  ...
  if (!isTeacher && !isVerified) {
    ...
    if (!banner) {
      ...
      banner.style.cssText = 'background: linear-gradient(135deg, #451A03 0%, #7C2D12 100%); border: 1.5px solid #F59E0B; border-radius: 18px; padding: 18px 24px; margin: 20px 24px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px; backdrop-filter: blur(12px); box-shadow: 0 8px 30px rgba(245, 158, 11, 0.25); animation: fadeIn 0.3s ease; position: relative; z-index: 50;';
      banner.innerHTML = `
        <div style="display: flex; align-items: center; gap: 14px;">
          <div style="width: 48px; height: 48px; background: rgba(245, 158, 11, 0.2); border: 1px solid rgba(245, 158, 11, 0.4); border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 26px; flex-shrink: 0;">
            📧
          </div>
          <div>
            <strong style="color: #FDE68A; font-size: 16px; font-weight: 800; display: block; margin-bottom: 3px;">لم يتم تأكيد البريد الإلكتروني بعد! ⚠️</strong>
            <div style="font-size: 13.5px; color: #FFFFFF; font-weight: 700; margin-bottom: 2px;">اضغط لإرسال رسالة تأكيد البريد الإلكتروني</div>
            <div style="font-size: 12px; color: #E2E8F0; opacity: 0.9;">يرجى مراجعة صندوق الوارد (Inbox) أو مجلد الرسائل غير المرغوب فيها (Spam / Junk Mail) لفتح رابط التفعيل.</div>
          </div>
        </div>
        <div style="display: flex; align-items: center; gap: 10px;">
          <button onclick="resendEmailVerificationPrompt()" id="resend-email-btn" style="background: linear-gradient(135deg, #F59E0B 0%, #D97706 100%); color: #0F172A; border: none; padding: 12px 22px; border-radius: 12px; font-size: 13.5px; font-weight: 800; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 4px 14px rgba(245, 158, 11, 0.4); font-family: inherit;">
            إرسال بريد التأكيد 📩
          </button>
        </div>
      `;
...
async function resendEmailVerificationPrompt() {
  ...
  try {
    ...
    alert('✅ تم إرسال رسالة التأكيد بنجاح إلى بريدك الإلكتروني!\n\nيرجى فتح صندوق الوارد (Inbox) أو مجلد الرسائل غير المرغوب فيها (Spam / Junk Mail) لضغط رابط التفعيل.');
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'إرسال بريد التأكيد 📩';
    }
  }
}
