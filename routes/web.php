 ...
 * هذا الملف مخصص لمسارات الويب التي تعيد صفحات HTML وتفاعلات المتصفح المباشرة.
 * 
 * المسارات الرئيسية المتاحة:
 * 1. GET /verify-email/{token}: رابط تفعيل البريد الإلكتروني الذي يصله الطالب في الإيميل (يُحدث email_verified_at).
 * 2. GET /reset-password/{token}: صفحة إدخال كلمة المرور الجديدة بعد النسيان.
 * 3. POST /reset-password/{token}: استقبال واستبدال كلمة المرور الجديدة وتحديثها في قاعدة البيانات.
 * 4. GET /api/payment/sandbox/{ref}: صفحة محاكي الدفع التجريبي (Sandbox Payment Page).
 * ====================================================
 */
